...
A: HIK (Hierarchical Isolation Kernel) 是一个新型的操作系统内核，通过统一的架构范式，构建一个能适应从资源受限的嵌入式设备到功能复杂的通用计算系统的操作系统内核。核心目标是在单一架构框架内实现极致性能、强安全隔离、动态可扩展性的三元统一。
...
A: HIK引导程序需要为UEFI环境编译，UEFI使用PE格式，需要特殊的交叉编译工具链。内核需要为特定架构编译，也需要相应的交叉编译器。
...
A: 能力（Capability）是HIK中所有授权的载体。能力是不可伪造的内核对象，描述对系统资源的特定访问权限（读、写、执行）。所有资源访问都必须通过能力验证。
...
A: 形式化验证使用数学方法证明系统的正确性。HIK的核心不变式都有对应的数学证明，确保系统的安全性和可靠性。
...
A: 是的，HIK支持多核处理器。核心-0可以管理多个CPU核心，并支持线程在不同核心上运行。

### Q: HIK支持热插拔吗？

A: 是的，HIK支持热插拔设备。热插拔协调服务负责处理设备插入和拔出事件。

### Q: HIK支持滚动更新吗？

A: 是的，HIK支持滚动更新。可以在线更新服务模块，无需重启系统。

详见 [滚动更新](./22-RollingUpdate.md)。

### Q: HIK支持容器吗？

A: HIK的设计本身就类似于容器，每个服务运行在独立的沙箱中。可以在此基础上实现更高层次的容器抽象。
...
A: HIK目前处于v0.1.0阶段，核心功能已实现，但还需要更多的测试和优化。计划在v1.0.0达到生产就绪状态。
