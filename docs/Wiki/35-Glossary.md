...
本文档定义了 HIK 系统中使用的关键术语和概念。

## 核心术语

### HIK (Hierarchical Isolation Kernel)
分层隔离内核。一个微内核操作系统内核，使用分层架构和数学形式验证来保证安全性。

### Core-0 (核心层)
HIK 的最底层特权层，负责系统核心功能：内存管理、能力系统、调度器、域切换等。

### Privileged-1 (特权层)
HIK 的中间特权层，提供设备驱动、文件系统、网络栈等特权服务。

### Application-3 (应用层)
HIK 的最高层用户空间，运行应用程序，通过系统调用和IPC与内核交互。
