...
HIK (Hierarchical Isolation Kernel，分级隔离内核) 是一个新型的操作系统内核，通过统一的架构范式，构建一个能适应从资源受限的嵌入式设备到功能复杂的通用计算系统的操作系统内核。

## 核心目标

HIK的核心目标是在单一架构框架内实现以下三个目标的统一：
...
| 特性 | Linux宏内核 | seL4微内核 | Windows混合内核 | HIK分级隔离内核 |
|------|-------------|-----------|-----------------|-----------------|
