...
HIK_IMG_MAGIC = b"HIK_IMG"
HIK_IMG_VERSION = 1

def create_hik_image(elf_path, output_path):
    ...
    header[0:8] = HIK_IMG_MAGIC
    struct.pack_into('<H', header, 8, 1)
    struct.pack_into('<H', header, 10, HIK_IMG_VERSION)
...
if __name__ == '__main__':
    ...
    if not create_hik_image(sys.argv[1], sys.argv[2]):
        sys.exit(1)
