...
cd build && objcopy -O binary bin/hic-kernel.elf bin/hic-kernel.bin > /dev/null 2>&1 && cd ..
cp build/bin/hic-kernel.bin output/ > /dev/null 2>&1
rm -f output/hic-uefi-disk.img
dd if=/dev/zero of=output/hic-uefi-disk.img bs=1M count=64 2>/dev/null
mkfs.fat -F 32 output/hic-uefi-disk.img > /dev/null 2>&1
mmd -i output/hic-uefi-disk.img ::EFI > /dev/null 2>&1
mmd -i output/hic-uefi-disk.img ::EFI/BOOT > /dev/null 2>&1
mcopy -i output/hic-uefi-disk.img output/bootx64.efi ::EFI/BOOT/BOOTX64.EFI > /dev/null 2>&1
mcopy -i output/hic-uefi-disk.img output/hic-kernel.bin ::hic-kernel.bin > /dev/null 2>&1
echo "✓ 磁盘镜像准备完成"
...
# 关键指标
BOOTLOADER_OK=$(grep -c "HIC UEFI Bootloader" /tmp/serial_test.log 2>/dev/null || echo 0)
...
HELLO_HIC=$(grep -c "Hello HIC" /tmp/serial_test.log 2>/dev/null || echo 0)
...
echo "[Bootloader阶段]"
grep -E "HIC UEFI|Starting HIC|Auto-boot|Kernel loaded" /tmp/serial_test.log | tail -10
echo ""
...
if [ $HELLO_HIC -gt 0 ]; then
    echo "✅ 内核成功输出 Hello HIC"
else
    echo "❌ 内核没有输出 Hello HIC"
fi
