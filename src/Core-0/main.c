...
void kernel_main(kernel_info_t *info)
{
    ...
    /* 1. 初始化物理内存管理器 */
    console_puts("[2/9] Initializing Physical Memory Manager...\n");
    pmm_init();
    ...
    /* 2. 初始化能力系统 */
    console_puts("[3/9] Initializing Capability System...\n");
    capability_system_init();
    
    /* 3. 初始化中断控制器 */
    console_puts("[4/9] Initializing IRQ Controller...\n");
    irq_controller_init();
    
    /* 4. 初始化域管理器 */
    console_puts("[5/9] Initializing Domain Manager...\n");
    domain_system_init();
    
    /* 5. 初始化线程系统 */
    console_puts("[6/9] Initializing Thread System...\n");
    thread_system_init();
    
    /* 6. 初始化调度器 */
    console_puts("[7/9] Initializing Scheduler...\n");
    scheduler_init();
    
    /* 7. 创建Core-0自身域 */
    console_puts("[8/9] Creating Core-0 Domain...\n");
